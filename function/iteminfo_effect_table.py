LITERAL_REPLACE_TABLE = {
    #region 職業
    '전 직업': '所有職業',
    '노비스': '初學者',
    '검사': '劍士',
    '복사': '服事',
    '마법사': '魔法師',
    '상인': '商人',
    '궁수계열': '弓箭手',
    '도둑': '盜賊',
    '룬나이트': '盧恩騎士',
    '드래곤 나이트': '盧恩龍爵',
    '로얄가드': '皇家禁衛隊',
    '임페리얼 가드': '帝國聖衛軍',
    '아크비숍': '大主教',
    '카디날': '樞機主教',
    '수라': '修羅',
    '인퀴지터': '聖裁者',
    '워록': '咒術士',
    '아크메이지': '禁咒魔導士',
    '소서러': '妖術師',
    '엘레멘탈 마스터': '元素支配者',
    '미케닉': '機械工匠',
    '마이스터': '機甲神匠',
    '제네릭': '基因學者',
    '바이올로': '生命締造者',
    '레인저': '遊俠',
    '윈드호크': '風鷹狩獵者',
    '민스트럴': '宮廷樂師',
    '트루베르': '天籟頌者',
    '원더러': '浪跡舞者',
    '트루바두르': '樂之舞靈',
    '길로틴크로스': '十字斬首者',
    '쉐도우 크로스': '十字影武',
    '쉐도우체이서': '魅影追蹤者',
    '어비스 체이서': '深淵追跡者',
    '성제': '拳皇',
    '천제': '天帝',
    '소울리퍼': '獵靈士',
    '영도사': '契靈士',
    '리벨리온': '叛亂者',
    '나이트 워치': '夜行使',
    '카게로우': '影狼 (日影忍者)',
    '신키로': '流浪忍者',
    '오보로': '朧 (月影忍者)',
    '시라누이': '疾風忍者',
    '슈퍼노비스': '超級初學者',
    '하이퍼 노비스': '終極初學者',
    '소환사': '召喚師',
    '도람족': '喵族',
    '혼령사': '魂靈師',
    #endregion
    
	#region 技能
	'기본기능': '基本技能',
    '한손검 수련': '單手劍使用熟練度',
    '양손검 수련': '雙手劍使用熟練度',
    'HP회복력 향상': '快速恢復',
    '배쉬': '狂擊',
    '프로보크': '挑釁',
    '매그넘 브레이크': '怒爆',
    '인듀어': '霸體',
    'SP회복력 향상': '禪心',
    '사이트': '火狩',
    '네이팜 비트': '心靈爆破',
    '세이프티 월': '暗之障壁',
    '소울 스트라이크': '聖靈召喚',
    '콜드볼트': '冰箭術',
    '프로스트 다이버': '冰凍術',
    '스톤 커스': '石化術',
    '화이어 볼': '火球術',
    '화이어 월': '火焰之壁',
    '화이어 볼트': '火箭術',
    '라이트닝 볼트': '雷擊術',
    '선더스톰': '雷爆術',
    '디바인 프로텍션': '天使之護',
    '데몬베인': '天使之擊',
    '루아흐': '光獵',
    '흐뉴마': '光之障壁',
    '텔레포테이션': '瞬間移動',
    '워프 포탈': '傳送之陣',
    '힐': '治癒術',
    '민첩성 증가': '加速術',
    '민첩성 감소': '緩速術',
    '아쿠아 베네딕타': '天使之淚',
    '시그넘 크루시스': '天使之光',
    '안젤루스': '天使之障壁',
    '블레싱': '天使之賜福',
    '큐어': '治療術',
    '소지한계량 증가': '負重量上升',
    '디스카운트': '低價買進',
    '오버챠지': '高價賣出',
    '푸쉬카트': '手推車使用',
    '아이템 감정': '物品鑑定',
    '노점개설': '露天商店',
    '매머나이트': '金錢攻擊',
    '올빼미의 눈': '鶚梟之眼',
    '독수리의 눈': '蒼鷹之眼',
    '집중력 향상': '心神凝聚',
    '더블 스트레이핑': '二連矢',
    '애로우 샤워': '箭雨',
    '더블 어택': '二刀連擊',
    '회피율 증가': '殘影',
    '스틸': '偷竊',
    '하이딩': '隱匿',
    '인베넘': '施毒',
    '해독': '解毒',
    '리저렉션': '復活術',
    '창 수련': '長矛使用熟練度',
    '피어스': '連刺攻擊',
    '브랜디쉬 스피어': '騎乘攻擊',
    '스피어 스탭': '長矛刺擊',
    '스피어 부메랑': '投擲長矛攻擊',
    '투핸드 퀴큰': '雙手劍攻擊速度增加',
    '오토 카운터': '反擊',
    '볼링 배쉬': '怪物互擊',
    '라이딩': '騎乘術',
    '기병 수련': '騎兵修練',
    '메이스 수련': '權杖使用熟練度',
    '임포시티오 마누스': '神威祈福',
    '수프라기움': '犧牲祈福',
    '아스페르시오': '撒水祈福',
    '성체강복': '聖之祈福',
    '생츄어리': '光耀之堂',
    '슬로우 포이즌': '緩毒術',
    '리커버리': '痊癒術',
    '기리에 엘레이손': '霸邪之陣',
    '마니피캇': '聖母之頌歌',
    '글로리아': '幸運之頌歌',
    '렉스 디비나': '沉默之術',
    '턴 언데드': '轉生術',
    '렉스 에테르나': '天使之怒',
    '마그누스 엑소르시스무스': '十字驅魔攻擊',
    '화이어 필라': '火柱攻擊',
    '사이트래셔': '火之獵殺',
    '화이어 아이비': '火焰藤蔓',
    '메테오 스톰': '隕石術',
    '유피텔 선더': '雷鳴術',
    '로드 오브 버밀리온': '怒雷強擊',
    '워터볼': '水球術',
    '아이스월': '冰刃之牆',
    '프로스트 노바': '霜凍之術',
    '스톰 가스트': '暴風雪',
    '어스 스파이크': '地震術',
    '헤븐즈 드라이브': '崩裂術',
    '콰그마이어': '泥沼地',
    '몬스터 정보': '怪物情報',
    '철 제조': '鐵製造',
    '강철 제조': '鋼製造',
    '속성석 제조': '屬性石製造',
    '오리데오콘 연구': '神之金屬研究',
    '단검 제작': '短劍製作',
    '검 제작': '劍製作',
    '양손검 제작': '雙手劍製作',
    '도끼 제작': '斧頭製作',
    '메이스 제작': '權杖製作',
    '너클 제작': '拳套製作',
    '창 제작': '長矛製作',
    '힐트 바인딩': '武器保有',
    '광석발견': '尋找礦石',
    '무기연구': '武器研究',
    '무기 수리': '武器修理',
    '스킨 템퍼링': '強化火屬性',
    '해머폴': '大地之擊',
    '아드레날린 러쉬': '速度激發',
    '웨폰 퍼펙션': '無視體型攻擊',
    '오버 트러스트': '凶砍',
    '맥시마이즈 파워': '武器值最大化',
    '스키드 트랩': '滑動陷阱',
    '랜드 마인': '地雷陷阱',
    '앵클스네어': '定位陷阱',
    '쇼크웨이브 트랩': '魔耗陷阱',
    '샌드맨': '睡魔陷阱',
    '플래셔': '強光陷阱',
    '프리징 트랩': '霜凍陷阱',
    '블래스트 마인': '定時爆炸陷阱',
    '크레모어 트랩': '爆散陷阱',
    '리무브 트랩': '陷阱移除',
    '토키 박스': '陷阱探查',
    '비스트베인': '動物殺手',
    '팔콘리 마스터리': '馴鷹術',
    '스틸 크로': '鋼製喙',
    '블릿츠 비트': '閃電衝擊',
    '디텍팅': '獵鷹尋敵',
    '스프링 트랩': '爆破陷阱',
    '오른손 수련': '右手修練',
    '왼손 수련': '左手修練',
    '카타르 수련': '拳刃修練',
    '클로킹': '偽裝',
    '소닉 블로우': '音速投擲',
    '그림투스': '無影之牙',
    '인챈트 포이즌': '塗毒',
    '포이즌 리액트': '毒性反彈',
    '베넘 더스트': '病毒散撥',
    '베넘 스플래셔': '毒性感染',
    '응급 치료': '緊急治療',
    '죽은척하기': '裝死',
    '이동시 HP 회복': '移動時恢復HP',
    '급소치기': '攻擊弱點',
    '오토 버서크': '狂暴狀態',
    '화살 만들기': '製作箭',
    '챠지 애로우': '衝鋒箭',
    '모래 뿌리기': '噴砂',
    '백 슬라이딩': '後退迴避',
    '돌 줍기': '撿石頭',
    '돌 던지기': '投擲石頭',
    '카트 레볼루션': '手推車攻擊',
    '체인지 카트': '改裝手推車',
    '고성방가': '大聲吶喊',
    '홀리 라이트': '神聖之光',
    '에너지 코트': '能量外套',
    '디펜더': '光之盾',
    '스내쳐': '強奪',
    '스틸 코인': '偷錢',
    '백 스탭': '背刺',
    '터널 드라이브': '潛遁',
    '기습': '潛擊',
    '스트립 웨폰': '卸除武器',
    '스트립 쉴드': '卸除盾牌',
    '스트립 아머': '卸除鎧甲',
    '스트립 헬름': '卸除頭盔',
    '인티머데이트': '脅持',
    '그래피티': '塗鴉',
    '플래그 그래피티': '旗幟塗鴉',
    '클리너': '清洗',
    '갱스터 파라다이스': '流氓天國',
    '컴펄션 디스카운트': '強制減價',
    '도작': '抄襲',
    '도끼 수련': '斧頭使用熟練度',
    '러닝 포션': '知識藥水',
    '파머시': '配藥',
    '데몬스트레이션': '火煙瓶投擲',
    '애시드 테러': '強酸攻擊',
    '포션 피쳐': '藥水投擲',
    '바이오 캐니벌라이즈': '生物調撥',
    '스피어 마인': '氣泡蟲召喚',
    '케미칼 프로텍션(웨폰)': '化學武器保護',
    '케미칼 프로텍션(쉴드)': '化學盾牌保護',
    '케미칼 프로텍션(아머)': '化學鎧甲保護',
    '케미칼 프로텍션(헬름)': '化學頭盔保護',
    '생명윤리': '生命倫理',
    '생명공학 연구': '生命工學研究',
    '피조물 창조': '生命體',
    '컬터베이션': '培養',
    '프레임 컨트롤': '火焰控制',
    '콜 호문클루스': '生命體召喚',
    '안식': '安息',
    '드릴마스터': '攻擊力訓練',
    '힐 호문클루스': '治癒生命體',
    '리저렉션 호문클루스': '復活生命體',
    '믿음': '信任',
    '오토 가드': '自動防禦',
    '쉴드 챠지': '盾擊',
    '쉴드 부메랑': '迴旋盾擊',
    '리플렉트 쉴드': '反射盾',
    '홀리 크로스': '聖十字攻擊',
    '그랜드 크로스': '聖十字審判',
    '헌신': '犧牲',
    '신의 뜻': '神祐之光',
    '스피어 퀴큰': '長矛加速術',
    '철사장(鐵沙掌)': '鐵沙掌',
    '운기조식(運氣調息)': '運氣調息',
    '축기(畜氣)': '蓄氣',
    '흡기(吸氣)': '吸氣',
    '육합권(六合拳)': '六合拳',
    '궁신탄영(弓身彈影)': '弓身彈影',
    '이화접목(移花接木)': '移花接木',
    '침투경(浸透勁)': '浸透勁',
    '탄지신통(彈指神通)': '彈指神通',
    '금강불괴(金剛不壞)': '金剛不壞',
    '진검백파도(眞劍百破道)': '真劍百破道',
    '폭기(爆氣)': '爆氣',
    '아수라 패황권(阿修羅覇凰拳)': '阿修羅霸凰拳',
    '연환전신장(連環全身掌)': '連環全身掌',
    '맹룡과강(猛龍誇强)': '猛龍誇強',
    '어드밴스드 북': '進化之書',
    '캐스트 캔슬': '取消施法',
    '매직 로드': '魔法懲罰',
    '스펠 브레이커': '念咒拆除',
    '프리 캐스트': '自由施法',
    '오토 스펠': '自動念咒',
    '프레임 런처': '火屬性附加',
    '프로스트 웨폰': '水屬性附加',
    '라이트닝 로더': '風屬性附加',
    '사이즈믹 웨폰': '地屬性附加',
    '드래고놀로지': '龍知識',
    '볼케이노': '火元素領域',
    '델류즈': '水元素領域',
    '바이어런스 게일': '風元素領域',
    '랜드 프로텍터': '地元素領域',
    '디스펠': '魔法效果解除',
    '아브라카다브라': '隨機技能',
    '임기응변': '臨機應變',
    '앵콜': '安可',
    '자장가': '搖籃曲',
    '김서방 돈벌었네': '經驗值倍增',
    '영원의 혼돈': '永遠的混沌',
    '전장의 북소리': '戰鼓震天',
    '니벨룽겐의 반지': '尼貝隆根之戒指',
    '울부짖는 로키': '洛奇的悲鳴',
    '심연속으로': '觸媒之所',
    '불사신 지크프리드': '不死神齊格弗里德',
    '라그나로크': '仙境傳說',
    '악기다루기': '操控樂器',
    '뮤지컬 스트라이크': '樂器攻擊',
    '불협화음': '不諧和音',
    '썰렁한 농담': '冷笑話',
    '휘파람': '吹口哨',
    '석양의 어쌔신 크로스': '刺客的黃昏',
    '브라기의 시': '布萊奇之詩',
    '이둔의 사과': '伊登的蘋果',
    '춤연습': '練習舞蹈',
    '화살 감아 던지기': '纏箭投擲',
    '막춤': '醜陋之舞',
    '비명지르기': '驚聲尖叫',
    '흥얼거림': '哼唱之音',
    '나를 잊지 말아요': '勿忘我',
    '행운의 키스': '女神之吻',
    '당신을 위한 서비스': '為您服務',
    '당신만은 지킬게요': '只呵護妳',
    '당신을 위해서 희생할게요': '只為你犧牲',
    '당신이 보고싶어요': '想念你',
    '토마호크 던지기': '投擲風靈之斧',
    '오라 블레이드': '靈氣劍',
    '패링': '雙劍挌擋',
    '컨센트레이션': '集中攻擊',
    '텐션 릴렉스': '極速回復',
    '버서크': '狂怒之槍',
    '아숨프티오': '聖母之祈福',
    '바실리카': '神聖殿堂',
    '메디타티오': '冥想',
    '소울 드레인': '吸魂術',
    '매직 크래셔': '魔擊術',
    '마법력 증폭': '魔力增幅',
    '프레셔': '神之威壓',
    '세크리파이스': '捨命攻擊',
    '가스펠': '聖音',
    '맹호경파산 (猛虎硬派山)': '猛虎硬爬山',
    '복호권 (伏虎拳)': '伏虎拳',
    '연주붕격 (聯柱崩擊)': '氣絕崩擊',
    '라이프 치환': 'HP轉換',
    '소울 체인지': '心神互換',
    '소울 번': '精神耗弱術',
    '어드밴스드 카타르 연구': '高階拳刃修練',
    '치명적인 독 부여': '致命塗毒',
    '기공포': '心靈震波',
    '트루 사이트': '狙殺瞄準',
    '팔콘 어절트': '獵鷹突擊',
    '샤프슈팅': '銳利射擊',
    '윈드 워크': '風之步',
    '멜트다운': '野蠻凶砍',
    '동전제조': '金錢鑄造',
    '괴(塊) 제조': '金屬塊製造',
    '카트 부스트': '手推車加速',
    '자동공격장치 제작': '攻擊塔製作',
    '체이스 워크': '暗影追蹤',
    '소드 리젝트': '霸王魂',
    '배낭 털기': '背包偷竊',
    '알케미': '融合試驗',
    '포션 시놉시스': '藥劑試驗',
    '애로우 발칸': '奧義箭亂舞',
    '달빛 물레방앗간에 떨어지는 꽃잎': '落花伴著月光下的水車小屋',
    '마리오네트 컨트롤': '傀儡師的把戲',
    '스파이럴 피어스': '螺旋擊刺',
    '헤드 크러쉬': '傷害增壓',
    '조인트 비트': '巧打',
    '네이팜 발칸': '念力連擊',
    '광축기(狂蓄氣)': '狂蓄氣',
    '마인드 브레이커': '精神撼動',
    '메모라이즈': '速讀術',
    '월 오브 포그': '薄霧牆',
    '스파이더 웹': '易燃之網',
    '메테오 어절트': '黑暗瞬間',
    '치명적인 독 생산': '毒液製作',
    '엄마 아빠 사랑해요': '爸媽我愛您',
    '엄마 아빠 보고싶어요': '爸媽我想念您',
    '아가야 이리오렴': '寶貝請來這裡',
    '달리기': '跑步',
    '회오리 준비': '迴旋準備',
    '회오리차기': '迴旋踢',
    '찍기 준비': '砸踢準備',
    '내려찍기': '砸踢',
    '차기 준비': '踢準備',
    '돌려차기': '轉身踢',
    '카운터 준비': '還擊準備',
    '카운터차기': '還擊踢',
    '낙법': '落法',
    '날라차기': '飛腳踢',
    '편안한 휴식': '平安的休息',
    '즐거운 휴식': '快樂的休息',
    '화이팅': '加油',
    '따듯한 바람': '溫暖的風',
    '높이뛰기': '跳高',
    '태양과 달과 별의 느낌': '太陽和月亮和星星的感覺',
    '태양의 따스함': '太陽的溫暖',
    '달의 따스함': '月亮的溫暖',
    '별의 따스함': '星星的溫暖',
    '태양의 안락함': '太陽的平安感',
    '달의 안락함': '月亮的平安感',
    '별의 안락함': '星星的平安感',
    '태양과 달과 별의 증오': '太陽和月亮和星星的憎惡',
    '태양의 분노': '太陽的憤怒',
    '달의 분노': '月亮的憤怒',
    '별의 분노': '星星的憤怒',
    '태양의 축복': '太陽的祝福',
    '달의 축복': '月亮的祝福',
    '별의 축복': '星星的祝福',
    '태양과 달과 별의 악마': '太陽和月亮和星星的惡魔',
    '태양과 달과 별의 친구': '太陽和月亮和星星的朋友',
    '태양과 달과 별의 지식': '太陽和月亮和星星的知識',
    '태양과 달과 별의 융합': '太陽和月亮和星星的融合',
    '알케미스트의 영혼': '鍊金術師的靈魂',
    '버서크 피쳐': '菠色克投擲',
    '몽크의 영혼': '武道家的靈魂',
    '권성의 영혼': '拳聖的靈魂',
    '세이지의 영혼': '賢者的靈魂',
    '크루세이더의 영혼': '十字軍的靈魂',
    '슈퍼노비스의 영혼': '超級初學者的靈魂',
    '기사의 영혼': '騎士的靈魂',
    '위저드의 영혼': '巫師的靈魂',
    '프리스트의 영혼': '祭司的靈魂',
    '바드와 댄서의 영혼': '吟遊詩人和舞孃的靈魂',
    '로그의 영혼': '流氓的靈魂',
    '어쌔신의 영혼': '刺客的靈魂',
    '블랙스미스의 영혼': '鐵匠的靈魂',
    '풀 아드레날린 러쉬': '所有速度激發',
    '헌터의 영혼': '獵人的靈魂',
    '소울링커의 영혼': '悟靈士的靈魂',
    '카이젤': '凱易哲',
    '카아히': '凱阿希',
    '카우프': '凱誣僕',
    '카이트': '凱易特',
    '카이나': '凱易娜',
    '에스틴': '艾斯提',
    '에스턴': '艾斯敦',
    '에스마': '艾斯麻',
    '에스우': '艾斯誣',
    '에스크': '艾斯克',
    '에스카': '艾斯卡',
    '프리저브': '自由保護',
    '풀 스트립': '所有卸除',
    '무기제련': '武器精練',
    '슬림 피쳐': '纖細藥水投擲',
    '풀 케미칼 프로텍션': '所有化學武器保護',
    '쉴드체인': '連續盾擊',
    '마나 리차지': '魔力減免',
    '더블 캐스팅': '雙倍投擲',
    '간반테인': '咖般塔音',
    '그라비테이션 필드': '重力原野',
    '카트 터미네이션': '手推車終結技',
    '오버트러스트 맥스': '凶砍最大值',
    '헤르모드의 지팡이': '海羅默德的手杖',
    '운명의 타로 카드': '命運的塔羅牌',
    '애시드 데몬스트레이션': '強酸火煙瓶投擲',
    '식물 재배': '植物栽培',
    '태권 미션': '太拳任務',
    '일차상위직의 영혼': '一轉上等職業的靈魂',
    '원핸드 퀴큰': '單手劍攻擊速度增加',
    '트와이라이트 파머시': '寬廣配藥',
    '비스트 스트레이핑': '動物猛擊',
    '플립 더 코인': '裝滿硬幣',
    '플링': '投擲硬幣',
    '트리플 액션': '三連攻擊',
    '불스 아이': '公牛之眼',
    '매드니스 캔슬러': '瘋狂凱斯樂',
    '애드저스트먼트': '終極閃躲',
    '인크리징 어큐러시': '命中率遞增',
    '매지컬 블릿': '魔術彈',
    '크래커': '轟然巨響',
    '싱글 액션': '單槍射擊',
    '스네이크 아이': '瞄準之眼',
    '체인 액션': '連鎖衝擊',
    '트래킹': '百步穿楊',
    '디스암': '拋戈卸甲',
    '피어싱 샷': '霹靂彈',
    '래피드 샤워': '五連擊',
    '데스페라도': '亡命之徒',
    '개틀링 피버': '格林狂熱',
    '더스트': '彈片四射',
    '풀 버스터': '全面破壞',
    '스프레드 어택': '火力全開',
    '그라운드 드리프트': '四面埋伏',
    '비도 수련': '飛刀修練',
    '수리검 던지기': '投擲飛鏢',
    '쿠나이 던지기': '投擲飛刀',
    '풍마수리검 던지기': '投擲風魔飛鏢',
    '돈 던지기': '投擲金錢',
    '다다미 뒤집기': '榻榻米攻擊',
    '안개 베기': '霧裡砍劈',
    '그림자 도약': '影子跳躍',
    '그림자 베기': '隱身攻擊',
    '매미허물 벗기': '金蟬脫殼',
    '환영분신': '幻影分身',
    '인법 수련': '忍術修練',
    '홍염화(紅炎華)': '火炎花',
    '화염진(火炎陣)': '火炎陣',
    '폭염룡(爆炎龍)': '爆炎龍',
    '빙섬창(氷閃槍)': '冰閃槍',
    '수둔(水遁)': '水鈍',
    '빙정락(氷晶落)': '冰晶落',
    '풍인(風刃)': '風刃',
    '뇌격쇄(雷擊碎)': '雷擊碎',
    '삭풍(朔風)': '朔風',
    '염(念)': '念',
    '일섬': '一閃擊',
    '어스퀘이크': '地震連繫',
    '드래곤 피어': '天龍恐懼',
    '펄스 스트라이크': '脈震衝撞',
    '헬 저지먼트': '死亡天譴',
    '메스 사일런스': '廣範圍沉默',
    '메스 프리징': '廣範圍冰凍',
    '메스 블리딩': '廣範圍出血',
    '메스 스톤커스': '廣範圍石化',
    '메스 컨퓨젼': '廣範圍混亂',
    '메스 슬립': '廣範圍睡眠',
    '광역 사이트': '廣範圍火狩',
    '이블 랜드': '邪降',
    '매직 미러': '魔鏡',
    '슬로우 캐스트': '減緩詠唱',
    '치명적인 상처': '致命傷口',
    '스톤 스킨': '鋼筋鐵骨',
    '안티 매직': '反魔法',
    '메스 커즈': '廣範圍詛咒',
    '메스 스턴': '廣範圍暈眩',
    '뱀파이어릭 터치': '吸血鬼接觸',
    '마나 번': '法力燃燒',
    '소지량 증가R': '負重量上升R',
    '헬의 권능': '地獄之權能',
    '라이프스트림': '生命之流',
    '잘자라 우리아가': '搖籃曲',
    '광역 공포': '廣範圍恐怖',
    '광역 발화': '廣範圍著火',
    '광역 빙결': '廣範圍冰封',
    '광역 냉동': '廣範圍冷凍',
    '광역 깊은잠': '廣範圍沉睡',
    '광역 매혹': '廣範圍蠱惑',
    '독안개': '毒霧',
    '밀레니엄 쉴드': '千年神盾',
    '유성우': '流星雨',
    '얼음 지뢰': '寒冰地雷',
    '플레임 크로스': '火焰十字架',
    '맥스 페인': '極限痛苦',
    '프로스트 피어싱': '冰凍穿孔',
    '광역 스파이더 웹': '廣範圍易燃之網',
    '광역 흡혈': '廣範圍吸血',
    '화염 폭풍': '火焰風暴',
    '진동 잔향': '殘響',
    '광범위 렉스 에테르나': '廣泛天使之怒',
    '애로우 스톰': '箭雨風暴',
    '주박진': '咒縛陣',
    '드래곤 브레스': '龍之氣息',
    '페이탈 메나스': '致命威脅',
    '용암 분출': '岩漿噴發',
    '하울링 오브 만드라고라': '曼陀羅魔花的尖叫',
    '사이킥 웨이브': '超自然波',
    '레이 오브 제네시스': '創世之光',
    '클라우드 킬': '雲殺',
    '이그니션 브레이크': '致命爆裂',
    '할루시네이션 워크': '幻影步',
    '일렉트릭워크': '電流步',
    '파이어 워크': '火焰步',
    '죽음의 손아귀': '亡靈掌心',
    '광역 죽음의 손아귀': '廣範圍亡靈掌心',
    '올스텟 다운': '全素質下降',
    '중력 증가': '重力增加',
    '회복 전환': '轉禍為福',
    '이뮨 엘레멘탈': '屬性免疫',
    '위치 변경': '位置轉換',
    '챠지 어택': '衝鋒攻擊',
    '슈링크': '退縮',
    '소닉 액셀레이션': '超音速投擲',
    '베넘 나이프': '毒刃',
    '클로즈 컨파인': '緊密的約束',
    '사이트 블래스터': '火狩芽',
    '컨버터 제조': '元素肯貝特製作',
    '엘레멘탈 체인지 (수)': '元素更換(水)',
    '판타즈믹 애로우': '幻影箭',
    '팽 보이스': '陣痛之聲',
    '매혹의 윙크': '眨眼之誘',
    '수상쩍은 상술': '詭計的商術',
    '탐욕': '貪婪',
    '레뎀프티오': '捨身取義',
    '진기주입(振氣注入)': '振氣注入',
    '발경(發勁)': '發勁',
    '엘레멘탈 체인지 (지)': '元素更換(地)',
    '엘레멘탈 체인지 (화)': '元素更換(火)',
    '엘레멘탈 체인지 (풍)': '元素更換(風)',
    '엔첸트 블레이드': '魔力劍',
    '소닉 웨이브': '音速衝擊波',
    '데스 바운드': '死亡反彈',
    '헌드레드 스피어': '百矛穿刺',
    '윈드 커터': '風壓飛刃',
    '드래곤 트레이닝': '龍駕馭',
    '드래곤 하울링': '龍之咆哮',
    '룬 마스터리': '盧恩精熟',
    '크러쉬 스트라이크': '重擊強襲',
    '리프레쉬': '恢復',
    '자이언트 그로쓰': '力量成長',
    '스톤하드 스킨': '岩石皮膚',
    '바이탈리티 액티베이션': '生命激化',
    '스톰 블래스트': '風暴衝擊',
    '파이팅 스피릿': '提升鬥志',
    '어번던스': '豐足',
    '팬텀 스러스트': '幻象突刺',
    '베놈 임프레스': '毒耐性弱化',
    '크로스 임팩트': '十字斬',
    '다크 일루젼': '黑色幻影',
    '새로운 독 연구': '新毒研究',
    '새로운 독 제조': '新毒製作',
    '안티 도트': '解毒劑',
    '포이즈닝 웨폰': '劇毒武器',
    '웨폰 블로킹': '武器抵禦',
    '카운터 슬래쉬': '反擊斬',
    '웨폰 크러쉬': '卸除武裝',
    '베놈 프렛셔': '劇毒強制賦予',
    '포이즌 스모크': '毒霧',
    '클로킹 익시드': '偽裝強化',
    '팬텀 메나스': '惡靈威脅',
    '롤링 커터': '迴旋刀刃',
    '크로스 리퍼 슬래셔': '迴旋十字斬',
    '쥬덱스': '審判',
    '안실라': '安希拉',
    '아도라무스': '謳歌',
    '크레멘티아': '慈悲術',
    '칸토캔디두스': '純白百合花',
    '콜루세오힐': '燦爛聖光',
    '에피클레시스': '聖靈降臨祈禱',
    '프라에파티오': '感恩歌',
    '오라티오': '祈禱文',
    '라우다아그누스': '羔羊歌頌',
    '라우다라무스': '折枝讚頌',
    '에우카리스티카': '感恩祈禱',
    '레노바티오': '淨化',
    '하이네스 힐': '高階治癒術',
    '클리어런스': '解除',
    '엑스피아티오': '贖罪',
    '듀플레 라이트': '二道聖光',
    '시렌치움': '靜寂',
    '화이트 임프리즌': '白色監獄',
    '소울 익스펜션': '靈魂爆炸',
    '프로스트 미스티': '寒冰濃霧',
    '잭 프로스트': '凍僵術',
    '마쉬 오브 어비스': '深淵沼地',
    '리코그나이즈드 스펠': '魔法省悟',
    '시에나 엑서크레이트': '石化詛咒',
    '라디어스': '半徑擴大',
    '스테이시스': '魔力凍結',
    '드레인 라이프': '吸星大法',
    '크림즌 록': '碧血隕石',
    '헬 인페르노': '地獄火焰',
    '커미트': '毀滅慧星',
    '체인 라이트닝': '連鎖電擊',
    '어스 스트레인': '地牛翻身',
    '테트라 볼텍스': '屬性漩渦',
    '서몬 파이어볼': '召喚火焰球',
    '서몬 볼 라이트닝': '召喚雷電球',
    '서몬 워터볼': '召喚水球',
    '서몬 스톤': '召喚石塊',
    '릴리즈': '釋放',
    '리딩 스펠북': '閱讀魔法書',
    '프리징 스펠': '魔法保存',
    '피어브리즈': '微風恐懼',
    '레인저메인': '遊俠主意',
    '에임드 볼트': '瞄準標靶',
    '디토네이터': '雷管',
    '일렉트릭 쇼커': '電擊陷阱',
    '클러스터 봄': '榴霰彈',
    '워그마스터리': '召狼術',
    '워그라이더': '騎狼術',
    '워그 대쉬': '疾狼術',
    '워그 스트라이크': '狼突擊',
    '워그 바이트': '狼咬',
    '투스 오브 워그': '狼牙',
    '예민한 후각': '敏銳嗅覺',
    '카무플라쥬': '偽裝戰術',
    '트랩연구': '陷阱研究',
    '마젠타 트랩': '紫紅陷阱',
    '코발트 트랩': '深藍陷阱',
    '메이즈 트랩': '淺黃陷阱',
    '버듀어 트랩': '青翠陷阱',
    '파이어링 트랩': '燃燒陷阱',
    '아이스바운드 트랩': '冰封陷阱',
    '마도 기어 라이센스': '魔導機甲執照',
    '부스트 너클': '噴射飛拳',
    '파일 벙커': '衝擊椎',
    '발칸 암': '火神砲',
    '플레어 런처': '火焰噴射器',
    '콜드 슬로어': '液體冷卻彈',
    '암즈 캐논': '加農砲',
    '액셀레이션': '魔導機甲加速',
    '호버링': '懸停',
    '프론트 사이드 슬라이드': '大步前進',
    '백 사이드 슬라이드': '大步後退',
    '메인 프레임 개조': '主體改造',
    '셀프 디스트럭션': '自我摧毀',
    '셰이프 쉬프트': '形態轉換',
    '이머전시 쿨': '緊急冷卻',
    '인프라레드 스캔': '紅外線掃瞄',
    '애널라이즈': '解析',
    '마그네틱 필드': '磁場',
    '뉴트럴 배리어': '中性防護罩',
    '스텔스 필드': '隱形力場',
    '리페어': '修復',
    '불과 대지의 연구': '火焰與大地的研究',
    '액스 부메랑': '迴旋斧',
    '파워 스윙': '揮斧重擊',
    '액스 토네이도': '戰斧颶風',
    'FAW 실버 스나이퍼': 'FAW銀光狙擊手',
    'FAW 매직 디코이': 'FAW魔法傀儡',
    'FAW 해제': 'FAW解體',
    '리프로듀스': '繁殖',
    '오토 섀도우 스펠': '自動魅影念咒',
    '섀도우 폼': '魅影形態',
    '트라이앵글 샷': '三角射擊',
    '바디 페인팅': '人體彩繪',
    '인비지빌리티': '透明術',
    '데들리 인펙트': '致命感染',
    '마스커레이드-이너베이션': '面具:無力',
    '마스커레이드-그루미': '面具-憂鬱',
    '마스커레이드-이그노어런스': '面具-無知',
    '마스커레이드-레이지네스': '面具:懶散',
    '마스커레이드-언럭키': '面具-不幸',
    '마스커레이드-위크니스': '面具-衰弱',
    '스트립 액세서리': '卸除配件',
    '맨홀': '人孔',
    '디멘션 도어': '異次元之門',
    '카오스 패닉': '混沌恐慌',
    '마엘슈트롬': '漩渦',
    '블러디 러스트': '血腥欲望',
    '페인트 봄': '虛擊炸彈',
    '캐논 스피어': '加農砲攻擊',
    '배니싱 포인트': '放逐攻擊',
    '트램플': '摧殘',
    '쉴드 프레스': '重壓盾擊',
    '리플렉트 데미지 리덕션': '聖冕加護',
    '핀 포인트 어택': '精準攻擊',
    '포스 오브 뱅가드': '先鋒部隊',
    '레이지 버스트 어택': '憤怒突擊',
    '쉴드 스펠': '盾咒',
    '익시드 브레이크': '強化衝擊',
    '오버 브랜드': '支配烙印',
    '프레스티지': '威信',
    '밴딩': '聚集',
    '문 슬래셔': '半月斬',
    '파이어티': '虔誠',
    '어스 드라이브': '大地毀滅',
    '헤스페루스 리트': '黃昏星之光',
    '인스피레이션': '靈感',
    '쌍룡각(雙龍脚)': '雙龍腳',
    '천라지망(天羅地網)': '天羅地網',
    '지뢰진(地雷震)': '地雷震',
    '대전붕추(大纏崩墜)': '大纏崩墜',
    '호포(號砲)': '虎砲',
    '폭기산탄(爆氣散彈)': '爆氣散彈',
    '파쇄주(破碎柱)': '破碎柱',
    '주박진(呪縛陣)': '咒縛陣',
    '섬전보(閃電步)': '閃電步',
    '수라신탄(修羅身彈)': '修羅身彈',
    '선풍퇴(旋風腿)': '旋風腿',
    '잠룡승천(潛龍昇天)': '潛龍昇天',
    '흡기공(吸氣攻)': '吸氣攻',
    '전기주입(全氣注入)': '全氣注入',
    '나찰파황격(羅刹破凰擊)': '羅剎破凰擊',
    '점혈-묵(點穴-默)': '點穴-默',
    '점혈-쾌(點穴-快)': '點穴-快',
    '점혈-구(點穴-球)': '點穴-球',
    '점혈-반(點穴-反)': '點穴-反',
    '점혈-활(點穴-活)': '點穴-活',
    '스윙 댄스': '搖擺舞',
    '연인들을 위한 심포니': '戀人交響樂',
    '달빛의 세레나데': '月光小夜曲',
    '풍차를 향해 돌격': '朝風車突擊',
    '메아리의 노래': '回音之歌',
    '하모나이즈': '和聲演奏',
    '레슨': '課程',
    '메탈릭 사운드': '重金屬音樂',
    '도미니온 임펄스': '支配動力',
    '서비어 레인스톰': '大暴雨',
    '나락의 노래': '絕望之歌',
    '세이렌의 목소리': '塞壬之音',
    '사망의 골짜기에서': '死亡峽谷',
    '안식의 자장가': '沉睡搖籃曲',
    '순환하는 자연의 소리': '循環的大自然之音',
    '불확정요소의 언어': '不確定要素的語言',
    '수줍은 하루의 우울': '羞怯一天的憂鬱',
    '그레이트 에코': '巨大共鳴',
    '마나의 노래': '魔力之歌',
    '워그와 함께 춤을': '與狼共舞',
    '사운드 오브 디스트럭션': '毀滅之聲',
    '새터데이 나이트 피버': '狂歡週末夜',
    '레라드의 이슬': '雷拉多露水',
    '멜로디 오브 싱크': '消沈旋律',
    '비욘드 오브 워 크라이': '戰嚎的彼端',
    '언리미티드 허밍 보이스': '無限哼唱聲',
    '일렉트릭 워크': '電流步',
    '스펠 피스트': '魔力拳',
    '어스 그레이브': '大地墳場',
    '다이아몬드 더스트': '鑽石星塵',
    '포이즌 버스터': '劇毒猛擊',
    '스트라이킹': '打擊強化',
    '워머': '加熱術',
    '바큠 익스트림': '極限空虛',
    '바레티르 스피어': '聖槍刺擊',
    '아를로': '搖籃曲',
    '정령 제어': '精靈控制',
    '서몬 아그니': '召喚火元素',
    '서몬 아쿠아': '召喚水精靈阿奎亞',
    '서몬 벤투스': '召喚風精靈梵圖斯',
    '서몬 테라': '召喚地精靈泰拉',
    '엘레멘탈 액션': '精靈激發',
    '4원소 분석': '元素分析',
    '정령 교감': '精靈交流',
    '정령 치유': '精靈治癒',
    '파이어 인시그니아': '火之紋章',
    '워터 인시그니아': '水之紋章',
    '윈드 인시그니아': '風之紋章',
    '어스 인시그니아': '地之紋章',
    '검 수련': '單手劍使用熟練度',
    '카트개조': '手推車改良',
    '카트 토네이도': '手推車龍捲風',
    '카트 캐논': '手推車加農砲',
    '가시나무 덫': '荊棘陷阱',
    '블러드 서커': '吸血植物',
    '스포어 익스플로젼': '爆炸孢子',
    '가시나무 벽': '荊棘之壁',
    '크레이지 위드': '瘋狂野草',
    '데모닉 파이어': '惡魔火焰',
    '파이어 익스펜션': '火焰擴散',
    '헬즈 플랜트': '地獄植物',
    '슬링 아이템': '道具投擲',
    '체인지 매터리얼': '素材變化',
    '믹스 쿠킹': '調配料理',
    '폭탄 제조': '炸彈製造',
    '스페셜 파머시': '專門配藥',
    '세크라멘트': '聖典',
    '사자후(獅子吼)': '獅子吼',
    '뇌광탄(雷光彈)': '雷光彈',
    '오딘의 리콜': '奧丁的召喚',
    '엘 디카스테스로의 귀환': '返回艾爾迪卡斯特',
    '수호자의 부름': '守護者的召喚',
    '오딘의 힘': '奧丁之神力',
    '수호의 빛': '守護之光',
    '체인지 카트 2': '改裝手推車II',
    '액티비티 힐': '活躍治癒術',
    '리치스 코인': '富翁的財貨',
    '매스 스파이럴': '絕對貫穿',
    '배니싱 버스터': '強制驅逐',
    '바인드 트랩': '暗黑地獄',
    '플리커': '閃爍信號',
    '섀터 스톰': '粉碎風暴',
    '이터널 체인': '無限連鎖',
    '퀵 드로우 샷': '瞬速子彈',
    '크림즌 마커': '血色烙印',
    '파이어 댄스': '殺戮暗舞',
    '하울링 마인': '破壞怒吼',
    '플래티넘 알터': '白金祭壇',
    '펄른 엔젤': '墮落天使',
    '라운드 트립': '圓桌舞蹈',
    '드래곤 테일': '魔獸擺尾',
    '파이어 레인': '火焰暴雨',
    '히트 배럴': '加速子彈',
    '안티 매터리얼 블래스트': '毀滅重擊',
    '슬러그 샷': '根源破壞',
    '해머 오브 갓': '天神憤怒',
    '달의 빛': '月亮光輝',
    '달의 자세': '月亮英姿',
    '만월각': '滿月腳',
    '별의 빛': '星星光輝',
    '별의 자세': '星星英姿',
    '삭월각': '朔月腳',
    '섬광각': '閃光腳',
    '성제강림': '拳皇降臨',
    '신성폭발': '新星爆炸',
    '우주의 자세': '宇宙英姿',
    '유성낙하': '流星殞落',
    '중력조절': '重力調整',
    '차원의 서': '次元之書',
    '창성의 서': '創星之書',
    '태양과 달과 별의 기록': '太陽和月亮和星星的紀錄',
    '태양과 달과 별의 정화': '太陽和月亮和星星的淨化',
    '태양의 빛': '太陽光輝',
    '태양의 자세': '太陽英姿',
    '태양폭발': '太陽爆炸',
    '홍염각': '紅焰腳',
    '골렘의 영혼': '巨人靈魂',
    '그림자의 영혼': '影子靈魂',
    '매의 영혼': '隼鷹靈魂',
    '요정의 영혼': '精靈靈魂',
    '사령 폭발': '邪靈爆炸',
    '사악한 영혼의 저주': '邪惡靈魂詛咒',
    '에스파': '艾斯帕',
    '에스하': '艾斯哈',
    '에스후': '艾斯核',
    '영혼 결속': '靈魂集結',
    '영혼 분열': '靈魂分裂',
    '영혼 수확': '靈魂收割',
    '영혼 순환': '靈魂循環',
    '영혼 축적': '靈魂蓄積',
    '영혼 폭발': '靈魂引爆',
    '영혼 에너지 연구': '靈魂能量研究',
    '카우트': '凱渥特',
    '암운': '暗雲',
    '십자 베기': '幽冥十字斬',
    '영혼 절단': '靈魂阻隔',
    '폭발 쿠나이': '炸彈飛刀',
    '팔방 쿠나이': '八方飛刀',
    '마구 던지기': '投擲魔幣',
    '풍마수리검 난화': '風魔飛鏢之飛舞',
    '마키비시': '投擲三角釘',
    '명경지수': '明鏡止水',
    '환술 - 잔상': '幻術 - 殘影',
    '환술 - 경악': '幻術 - 驚恐',
    '환술 - 주살': '幻術 - 咒死',
    '화부 : 염천': '火符:炎天',
    '빙부 : 취설': '冰符:吹雪',
    '풍부 : 청람': '風符:青嵐',
    '토부 : 강괴': '地符:剛塊',
    '술식 해방': '法術 - 釋放',
    '술식 전개': '法術 - 施展',
    '환술 - 환혹': '幻術 - 迷惑',
    '열 여섯번째의 밤': '十六夜',
    '그림자 밟기': '踏影',
    '허무의 그림자': '虛無飄妙之影',
    '그림자무사': '影子武士',
    '일그러진 초승달': '變形的上弦月',
    '흐린 달빛의 환상': '陰月的幻影',
    '불길한 붉은 달빛': '不祥的紅月',
    '스노우플립': '冰雪寒葉',
    '페오니마미': '粉紅厚葉',
    '사다귀': '筮答葵葉',
    '세계수의먼지': '世界樹之塵',
    '에클라쥬 귀환': '返回艾可拉珠',
    '포스 트랜스퍼': '轉移力量',
    '범위 리저렉션': '廣泛復活術',
    '죽은 자들의 세계로!': '前往尼芙菲姆!',
    '프론테라 귀환': '返回普隆德拉',
    '글레스트헤임 귀환': '返回克雷斯特漢姆',
    '타나토스 귀환': '返回達納托斯',
    '검은 손톱': '致命爪痕',
    '언리밋': '菁英狙擊',
    '환각 도핑': '幻覺禁藥',
    '드래곤 브레스 - 수': '龍之氣息-水',
    '럭스 아니마': '盧恩爆發',
    '풍요의 노래': '豐年頌',
    '정령의 결계': '精靈結界',
    '섬광연격(閃光連擊)': '閃光連擊',
    '긴급탈출': '緊急脫身',
    '오페르토리움': '奉獻頌',
    '텔레키네틱 인텐스': '終極念力',
    '왕의 가호': '王的恩典',
    '풀 스로틀': '烈火戰車',
    '새로운 기본 기능': '新基本技能',
    '깨물기': '狠咬',
    '숨기': '躲藏',
    '할퀴기': '抓傷',
    '웅크리기': '俯身',
    '도약': '跳躍',
    '혼 구슬': '靈魂魔珠',
    '대지의 힘': '大地力量',
    '개다래 줄기 창': '獼猴桃梗槍',
    '개박하 가루 뿌리기': '貓薄荷灑粉',
    '개박하 메테오': '貓薄荷隕石',
    '개다래 뿌리 휘감기': '獼猴桃根莖纏繞',
    '생명의 힘': '生命力量',
    '타로우의 상처': '白鼠創傷',
    '픽키 마구쪼기': '小雞啄擊',
    '아크라우스 대시': '捲甲蟲暴衝',
    '루나틱 당근난타': '瘋兔胡蘿蔔重擊',
    '바다의 힘': '海洋力量',
    '참치 대뱃살': '鮪魚肚肉',
    '참치 파티': '鮪魚派對',
    '새우 떼': '蝦群',
    '새우의 신선함': '活蝦',
    '심령어택': '靈魂攻擊',
    '무리의 힘': '喵喵威武',
    '세비지의 영혼': '野豬之魂',
    '히스': '嘶嘶聲',
    '냥 그라스': '喵喵草',
    '그루밍': '舔毛',
    '퓨어링': '咕嚕咕嚕',
    '맛있는 새우파티': '美味活蝦派對',
    '생명의 혼': '生命之魂',
    '냥냥': '喵喵',
    '대지의 혼': '大地之魂',
    '채터링': '喵喵不休',
    '바다의 혼': '海洋之魂',
    '우리가 함께라면': '當我們同在一起',
    '죽음도 우릴 갈라놓을 순 없어요': '生生世世永不分離',
    '엄마 아빠 힘내세요': '爸比媽咪請加油',
    '전문 음악인의 노련함': '職人演奏家',
    '비투페라툼': '天罰',
    '콘베니오': '集結',
    '브레이크쓰루': '突破極限',
    '천사님 도와주세요!': '救援天使!',
    '트렌센던스': '心靈超越',
    '서번트 웨폰': '死侍武器',
    '서번트 웨폰 - 사인': '死侍武器-標記',
    '서번트 웨폰 - 팬텀': '死侍武器-瞬幻',
    '서번트 웨폰 - 데몰리션': '死侍武器-破滅',
    '챠징 피어스': '蓄力刺擊',
    '투핸드 디펜딩': '雙向防禦',
    '핵 앤드 슬래셔': '橫揮斬',
    '드래고닉 오라': '天龍光環',
    '매드니스 크러셔': '狂暴粉碎',
    '비고르': '活力之源',
    '스톰 슬래쉬': '風暴斬擊',
    '데들리 프로젝션': '致命放射',
    '디스트럭티브 허리케인': '毀滅颶風',
    '레인 오브 크리스탈': '冰晶飛瀑',
    '미스터리 일루전': '神秘幻滅',
    '바일런트 퀘이크': '震裂術',
    '소울 발칸 스트라이크': '靈魂連擊',
    '스트레이텀 트래머': '地層震動',
    '올 블룸': '萬紫千紅',
    '크리스탈 임팩트': '水晶波爆',
    '토네이도 스톰': '龍捲風暴',
    '투핸드 스태프 마스터리': '雙手杖使用熟練度',
    '플로랄 플레어 로드': '焰火之徑',
    '아스트랄 스트라이크': '星際突襲',
    '클라이막스': '魔力巔峰',
    '록 다운': '巨石降臨',
    '스톰 캐논': '暴風加農砲',
    '크림즌 애로': '豔紅魔矢',
    '크림즌 애로우': '豔紅魔矢',
    '프로즌 슬래쉬': '冰刃斬擊',
    '강인한 신념': '堅毅信念',
    '굳건한 신념': '堅定信念',
    '믿음의 의지': '信念意志',
    '성유세례': '聖油洗禮',
    '신실한 신념': '真誠信念',
    '염화멸마신탄': '焰魔散彈',
    '폭화신탄': '爆破衝擊',
    '제1격 : 낙인': '第一擊:烙印',
    '제1장 : 신념의 힘': '首章:信仰之力',
    '제2장 : 심판자': '次章:審判者',
    '제2격 : 멸마지화': '第二擊:火焰滅絕',
    '제2격 : 신념': '第二擊:信念',
    '제2격 : 심판': '第二擊:審判',
    '제3격 : 단죄': '第三擊:懲治',
    '제3격 : 멸화격': '第三擊:火焰彈',
    '제3격 : 정화': '第三擊:淨化',
    '최종장 : 멸마의 불꽃': '最終章:驅魔火焰',
    '가드 스탠스': '防禦架式',
    '가디언 쉴드': '守護神盾',
    '리바운드 쉴드': '回彈神盾',
    '쉴드 마스터리': '防盾精熟',
    '스피어&소드 마스터리': '矛&劍精熟',
    '어택 스탠스': '攻擊架式',
    '얼티메이트 세크리파이스': '終極犧牲',
    '홀리 쉴드': '抗性聖盾',
    '그랜드 저지먼트': '末日審判',
    '저지먼트 크로스': '交錯審判',
    '쉴드 슈팅': '盾牌投擲',
    '오버 슬래쉬': '瞬間斬擊',
    '크로스 레인': '聖十字雨',
    '레파라티오': '聖光治癒',
    '메디알레 보툼': '治癒誓言',
    '메이스&북 마스터리': '鈍器&書籍精熟',
    '아르구투스 비타': '光耀天命',
    '아르구투스 텔룸': '神聖防護',
    '아르비트리움': '制裁謳歌',
    '프레센스 아치에스': '爆裂聖光',
    '피두스 아니무스': '靈魂忠誠',
    '에플리고': '破壞擊',
    '콤페텐티아': '神聖權能',
    '프네우마티쿠스 프로첼라': '驅魔風暴',
    '딜렉티오 힐': '博愛治癒',
    '렐리지오': '全心奉獻',
    '베네딕툼': '祝福讚歌',
    '페티티오': '聖光打擊',
    '플라멘': '聖靈鎮魂',
    '쉐도우 익시드': '魅影強化',
    '댄싱 나이프': '舞動血刃',
    '세비지 임팩트': '野蠻衝擊',
    '쉐도우 센스': '影刃旋舞',
    '이터널 슬래쉬': '永恆斬擊',
    '포텐트 베넘': '強效毒液',
    '쉐도우 스탭': '暗影刺擊',
    '임팩트 크레이터': '神力撞擊',
    '인챈팅 쉐도우': '迷惑魅影',
    '페이탈 쉐도우 크로우': '致命影爪',
    '액스 스톰프': '戰斧踏滅',
    '러쉬 퀘이크': '衝擊撼動',
    '장치 제조': '製造裝置',
    '공격 장치 활성화': '啟動攻擊裝置',
    '방어 장치 활성화': '啟動防禦裝置',
    '투 액스 디펜딩': '雙手斧防禦',
    'ABR 마스터리': 'ABR精熟',
    'ABR-배틀워리어': 'ABR-決戰勇士',
    'ABR-듀얼캐논': 'ABR-雙子加農砲',
    'ABR-마더넷': 'ABR-天網聖母',
    'ABR-인피니티': 'ABR-無限戰火',
    '대거&보우 마스터리': '短劍&弓弩精熟',
    '매직소드 마스터리': '魔劍精熟',
    '스트립 쉐도우': '卸除影子',
    '어비스 대거': '深淵匕首',
    '언럭키 러쉬': '不幸衝擊',
    '체인 리액션 샷': '連鎖射擊',
    '프롬 디 어비스': '萬丈深淵',
    '어비스 슬레이어': '深淵殺手',
    '오메가 어비스 스트라이크': '終結深淵攻擊',
    '데프트 스탭': '靈巧刺擊',
    '어비스 스퀘어': '深淵墳場',
    '프렌지 샷': '狂暴射擊',
    '어드밴스드 트랩': '高階陷阱',
    '윈드 사인': '八風標記',
    '자연 친화': '熱愛自然',
    '호크 러쉬': '獵鷹奇襲',
    '호크 마스터리': '獵鷹精熟',
    '캘러미티 가일': '憤怒暴風',
    '호크 부메랑': '獵鷹狙擊',
    '가일 스톰': '毀滅風暴',
    '딥 블라인드 트랩': '深盲陷阱',
    '솔리드 트랩': '磐石陷阱',
    '스위프트 트랩': '急速陷阱',
    '크레시브 볼트': '漸進狙擊',
    '플레임 트랩': '火焰陷阱',
    '바이오닉 파머시': '仿生配藥',
    '바이오닉 마스터리': '仿生精熟',
    '더 홀 프로텍션': '全面化學保護',
    '풀 쉐도우 프로텍션': '全影化學保護',
    '애시디파이드 존(수)': '強酸禁地(水)',
    '애시디파이드 존(지)': '強酸禁地(地)',
    '애시디파이드 존(풍)': '強酸禁地(風)',
    '애시디파이드 존(화)': '強酸禁地(火)',
    '크리에이팅 우든 워리어': '召喚神木戰士',
    '크리에이팅 우든 페어리': '召喚神木妖精',
    '크리에이팅 크리퍼': '召喚爬行者',
    '리서치 리포트': '研究報告',
    '크리에이팅 헬 트리': '召喚地獄樹',
    '무대 매너': '舞台禮儀',
    '회상': '美聲回顧',
    '미스틱 심포니': '神秘交響曲',
    '소나타 오브 크바시르': '克瓦希爾奏鳴曲',
    '로제 블라썸': '玫瑰箭矢',
    '리듬 슈팅': '節奏射擊',
    '메탈릭 퓨리': '金屬狂怒',
    '사운드 블렌드': '混聲烙印',
    '게페니아 녹턴': '葛帔尼亞夜曲',
    '로키의 변덕스러움': '洛基奏鳴曲',
    '마인워커 랩소디': '礦工狂想曲',
    '뮤지컬 인터루드': '音樂劇插曲',
    '저녁 노을의 세레나데': '暮色小夜曲',
    '죽은자들을 위한 진혼곡': '亡靈鎮魂歌',
    '프론테라 행진곡': '普隆德拉進行曲',
    '매직 북 마스터리': '魔法書精熟',
    '스펠 인챈팅': '咒力賦予',
    '액티비티 번': '能量燃燼',
    '인크리징 액티비티': '能量轉換',
    '다이아몬드 스톰': '鑽石風暴',
    '라이트닝 랜드': '雷霆陣地',
    '베놈 스웜프': '毒液沼澤',
    '컨프래그레이션': '灼熱岩漿',
    '테라 드라이브': '大地驅動',
    '엘레멘탈 스피릿 마스터리': '元素鬥志精熟',
    '서몬 엘레멘탈 아르도르': '召喚元素:阿爾多雷',
    '서몬 엘레멘탈 딜루비오': '召喚元素:迪盧比奧',
    '서몬 엘레멘탈 프로첼라': '召喚元素:普羅賽拉',
    '서몬 엘레멘탈 테레모투스': '召喚元素:泰雷莫圖斯',
    '서몬 엘레멘탈 세르펜스': '召喚元素:塞爾彭斯',
    '엘레멘탈 버스터': '元素破壞',
    '엘레멘탈 베일': '元素天幕',
    '그레네이드 마스터리': '榴彈精通',
    '집중 조준': '專注瞄準',
    '그레네이드 프래그먼트': '榴彈碎片',
    '더 비질랜트 엣 나이트': '夜裡的警惕',
    '온리 원 블릿': '獨一槍彈',
    '스파이럴 슈팅': '螺旋射擊',
    '매거진 포 원': '梅格競直擊',
    '와일드 파이어': '曠野之火',
    '베이직 그레네이드': '初階手榴彈',
    '헤이스티 파이어 인 더 홀': '洞中急火',
    '그레네이즈 드롭핑': '落空手榴彈',
    '오토 파이어링 런처': '自動射擊發射器',
    '히든 카드': '王牌出手',
    '미션 : 봄바드': '任務轟炸機',
    '부적 연마': '符咒研究',
    '영도술 연마': '引魂術研究',
    '수호부': '護身符',
    '무사부': '武士符',
    '법사부': '法師符',
    '수혼일신': '靈魂聚集',
    '서낭당': '城隍廟',
    '오행부': '五行符',
    '영도부': '竊魂平安符',
    '사령정화': '死靈淨化',
    '청룡부': '青龍符',
    '백호부': '白虎符',
    '주작부': '朱雀符',
    '현무부': '玄武符',
    '사방신부': '四方神符',
    '사방오행진': '四方五行陣',
    '천지신령': '天地神靈',
    '영물 이해': '靈物互動',
    '영물 교감 - 철호': '靈物交流: 鐵虎',
    '철호 허공할퀴기': '鐵虎空心爪',
    '철호 포효': '鐵虎咆哮',
    '호고공 난타': '虎高空亂打',
    '영물 교감 - 귀설': '靈物交流: 神龜',
    '귀설 물뿌리개': '神龜灑水砲',
    '귀설 바다축제': '神龜海洋慶典',
    '귀설 모래축제': '神龜沙雕節',
    '귀설 거칠게 요동치기': '神龜劇烈震盪',
    '영물 교감 - 현록': '靈物交流: 玄鹿',
    '현록의 오색뿔': '玄鹿五色角',
    '현록의 잎새바람': '玄鹿葉風',
    '현록포': '玄鹿砲',
    '급속 교감': '急速交流',
    '영물들의 축복': '靈物祝福',
    '독학-전투학': '自學策略',
    '독학-마도학': '自學魔法',
    '더블 볼링 배쉬': '雙重怪物互擊',
    '메가 소닉 블로우': '巨型音速投擲',
    '쉴드 체인 러쉬': '連續盾擊衝撞',
    '스파이럴 피어스 맥스': '螺旋擊刺Max',
    '메테오 스톰 버스터': '隕石術猛擊',
    '유피텔 선더 스톰': '雷鳴術風暴',
    '잭 프로스트 노바': '新霜凍術',
    '헬즈 드라이브': '地獄毀滅',
    '그라운드 그라비테이션': '地面重力',
    '네이팜 발칸 스트라이크': '念力連擊攻擊',
    '한계돌파': '突破極限',
    '룰 브레이킹': '突破規矩',
    '천기 수련': '天機修練',
    '병법 수련': '兵法修練',
    '천지일양': '天地一陽',
    '태천일양': '天晨一陽',
    '천양': '天陽',
    '천지일월': '天地一月',
    '태천일월': '天晨一月',
    '천월': '天月',
    '천지만성': '天地萬星',
    '천명낙성': '天命落星',
    '천성': '天星',
    '천라만상': '天羅萬象',
    '천기지신': '天氣地神',
    '녹아내리기': '致命融化',
    '스며들기': '滲透',
    '악몽지우기': '消除惡夢',
    '신기루': '海市蜃樓',
    '그림자 사냥': '詭影狩獵',
    '그림자 춤': '詭影舞動',
    '그림자 일섬': '詭影閃光',
    '풍마수리검 - 장악': '風魔飛鏢 - 掌握',
    '풍마수리검 - 구축': '風魔飛鏢 - 構築',
    '쿠나이 - 왜곡': '飛刀 - 扭曲',
    '쿠나이 - 회전': '飛刀 - 旋轉',
    '쿠나이 - 굴절': '飛刀 - 曲折',
    '적염포': '火焰砲',
    '냉혈포': '冷焰砲',
    '뇌전포': '雷電砲',
    '금룡포': '金龍砲',
    '암전포': '暗戰砲',
    '그림자 - 악몽': '詭影 - 噩夢',
    '비도 - 악몽': '飛刀 - 噩夢',
    '암흑룡 - 악몽': '暗黑龍 - 噩夢',
    '사색부': '四色符',
    '와일드 샷': '狂野射擊',
    '미드나잇 펄른': '午夜殞落',
    '천기신양': '天氣神陽 (暫譯)',
    '천기신월': '天氣神月 (暫譯)',
    '천성지행': '天星知行 (暫譯)',
    '위기 극복': '危機克服 (暫譯)',
    '철호 난무': '鐵虎亂舞 (暫譯)',
    '현록의 영력 발휘': '玄鹿的靈力發揮 (暫譯)',
    '드래고닉 브레스': '天龍氣息',
    '스파크 블래스터': '閃光衝擊波',
    '트리플 레이저': '三連雷射',
    '마이티 스매쉬': '強力粉碎',
    '익스플로시브 파우더': '爆炸炸藥',
    '메이헤믹 쏜즈': '混亂的荊棘',
    '드래고닉 피어스': '天龍擊刺 (暫譯)',
    '래디언트 스피어': '光輝刺擊',
    '임페리얼 크로스': '帝國十字斬',
    '임페리얼 프레셔': '帝國威壓 (暫譯)',
    '러쉬 스트라이크': '衝鋒斬擊',
    '파워풀 스윙': '爆發橫掃',
    '에너지 캐노네이드': '能量轟炸',
    '미스터리 파우더': '神秘粉末',
    '더스트 익스플로전': '星塵炸裂',
    '크로스 슬래쉬': '十字斬擊',
    '히트 앤드 슬라이딩': '突擊滑行',
    '체이싱 브레이크': '追襲突斷',
    '체이싱 샷': '追襲射擊',
    '어비스 플레임': '深淵火焰 (暫譯)',
    '에너지 컨버전': '能量轉換 (暫譯)',
    '사이킥 스트림': '超自然之流 (暫譯)',
    '디비누스 플로스': '神聖之花 (暫譯)',
    '열화기탄': '烈火氣彈 (暫譯)',
    '와일드 워크': '狂野步 (暫譯)',
    '리드미컬 웨이브': '節奏戰波',
	#endregion
    
	#region 裝備敘述
	', ': '、',
	',\n': '',
	'. ': '',
	'.\n': '\n',
    ' 추가 증가': '',
    
	'[등급별 추가 옵션]': '[各階級追加效果]',
	'[D등급]': '[D 階級]',
	'[C등급]': '[C 階級]',
	'[B등급]': '[B 階級]',
	'[A등급]': '[A 階級]',
    
    '계열 : 단검 ': '系列 : 短劍   ',
    '계열 : 메이스 ': '系列 : 鈍器   ',
    '계열 : 양손도끼 ': '系列 : 雙手斧   ',

    '계열 : 투구 ': '系列 : 頭飾   ',
    '계열 : 갑옷 ': '系列 : 鎧甲   ',
    '계열 : 방패 ': '系列 : 盾牌   ',
    '계열 : 걸칠것 ': '系列 : 披肩   ',
    '계열 : 신발 ': '系列 : 鞋子   ',
    '계열 : 액세서리(왼쪽) ': '系列 : 飾品(L)   ',
    '계열 : 액세서리(오른쪽) ': '系列 : 飾品(R)   ',
    '계열 : 카드': '系列 : 卡片',
    
	'장착 : 무기': '部位 : 武器',
	'장착 : 투구': '部位 : 頭飾',
    '장착 : 갑옷': '部位 : 鎧甲',
    '장착 : 방패': '部位 : 盾牌',
    '장착 : 걸칠것': '部位 : 披肩',
    '장착 : 신발': '部位 : 鞋子',
    '장착 : 액세서리': '部位 : 飾品',
    '장착 : 액세서리(왼쪽)': '部位 : 飾品(L)',
    '장착 : 액세서리(오른쪽)': '部位 : 飾品(R)',
    
    '공격 : ': '攻擊 : ',
    '방어 : ': '防禦 : ',
    '위치 : 상단 ': '部位 : 上段   ',
    '무게 : ': '重量 : ',
    '무기 레벨 : ': '武器等級 : ',
    '방어구 레벨 : ': '防具等級 : ',
    '요구 레벨 : ': '等級限制 : ',
    '장착 : ': '職業限制 : ',
    '계열': '系列',

    '무기에 대한 크기 패널티가 사라진다': '刪除武器體型懲罰',
	#endregion
    
}

FIRST_LAYER_REGEX_REPLACE_RULES = [
    #region 角色判斷
	{
		'pattern': r'(?P<indent>\s*)(?P<value>\d+) 레벨 이상인 경우,\s*',
		'replacement': '{indent}基本等級達 {value} 以上時，',
	},
	{
		'pattern': r'(?P<indent>\s*)베이스 레벨 (?P<value>\d+)이상인 경우,\s*',
		'replacement': '{indent}基本等級達 {value} 以上時，',
	},
	{
		'pattern': r'(?P<indent>\s*)(?P<value>\d+)레벨 이상인 경우,\s*',
		'replacement': '{indent}基本等級達 {value} 以上時，',
	},
	{
		'pattern': r'(?P<indent>\s*)순수 (?P<status>.+?)가 (?P<value>\d+) 이상인 경우,\s*',
		'replacement': '{indent}純粹 {status} 達 {value} 以上時，',
	},
	{
		'pattern': r'(?P<indent>\s*)순수 (?P<status>.+?)이 (?P<value>\d+) 이상인 경우,\s*',
		'replacement': '{indent}純粹 {status} 達 {value} 以上時，',
	},
	{
		'pattern': r'(?P<indent>\s*)순수 (?P<status>.+?) (?P<value>\d+) 이상인 경우,\s*',
		'replacement': '{indent}純粹 {status} 達 {value} 以上時，',
	},
    #endregion
    
    #region 精煉判斷
	{
		'pattern': r'(?P<indent>\s*)(?P<value>\d+)\s*제련 당\s*',
		'replacement': '{indent}精煉值每 +{value:>2} 時，',
	},
	{
		'pattern': r'(?P<indent>\s*)(?P<value>\d+)\s*제련 당,\s*',
		'replacement': '{indent}精煉值每 +{value:>2} 時，',
	},
	{
		'pattern': r'(?P<indent>\s*)무기 (?P<value>\d+)제련 당\s*',
		'replacement': '{indent}武器精煉值每 +{value:>2} 時，',
	},
	{
		'pattern': r'(?P<indent>\s*)(?P<value>\d+)제련 시,\s*',
		'replacement': '{indent}精煉值達 +{value:>2} 以上時，',
	},
	{
		'pattern': r'(?P<indent>\s*)(?P<value>\d+)제련 이상인 경우,\s*',
		'replacement': '{indent}精煉值達 +{value:>2} 以上時，',
	},
    #endregion
    
    #region 階級判斷
	{
		'pattern': r'(?P<indent>\s*)(?P<grade_level>.+?)등급 이상일 경우,\s*',
		'replacement': '{indent}階級達 {grade_level} 以上時，',
	},
	{
		'pattern': r'(?P<indent>\s*)무기의 등급이 (?P<grade_level>.+?)등급 이상인 경우,\s*',
		'replacement': '{indent}武器階級達 {grade_level} 以上時，',
	},
    #endregion
    
    #region 套裝判斷
	{
		'pattern': r'(?P<indent>\s*)(?P<equip>.+?)와 함께 장착시,\s*',
		'replacement': '{indent}同時裝備「{equip}」時，',
	},
	{
		'pattern': r'(?P<indent>\s*)(?P<equip>.+?)와 함께 장착 시,\s*',
		'replacement': '{indent}同時裝備「{equip}」時，',
	},
	{
		'pattern': r'(?P<indent>\s*)(?P<equip_1>.+?), (?P<equip_2>.+?)와 함께 장착 시,\s*',
		'replacement': '{indent}同時裝備「{equip_1}」與「{equip_2}」時，',
	},
	{
		'pattern': r'(?P<indent>\s*)(?P<equip_1>.+?), (?P<equip_2>.+?), (?P<equip_3>.+?)와 함께 장착 시,\s*',
		'replacement': '{indent}同時裝備「{equip_1}」、「{equip_2}」與「{equip_3}」時，',
	},
	{
		'pattern': r'(?P<indent>\s*)(?P<equip_1>.+?), (?P<equip_2>.+?), (?P<equip_3>.+?), (?P<equip_4>.+?)와 함께 장착 시,\s*',
		'replacement': '{indent}同時裝備「{equip_1}」、「{equip_2}」、「{equip_3}」與「{equip_4}」時，',
	},
	{
		'pattern': r'(?P<indent>\s*)장착한 (?P<equip_1>.+?), (?P<equip_2>.+?)의 등급이 각 (?P<grade_level>.+?)등급 이상일 경우,\s*',
		'replacement': '{indent}同時裝備「{equip_1}」與「{equip_2}」且階級均達 {grade_level} 階級以上時，',
	},
	{
		'pattern': r'(?P<indent>\s*)(?P<series_name>.+?)의 \((?P<series_detail>.+?)\)의 각 등급이 (?P<grade_level>.+?)등급 이상인 경우,\s*',
		'replacement': '{indent}{series_name}系列裝備 ({series_detail}) 的階級均達 {grade_level} 階級以上時，',
	},
    #endregion
    
    #region 其他判斷
	{
		'pattern': r'(?P<indent>\s*)(?P<level>.+?)레벨 무기인 경우,\s*',
		'replacement': '{indent}使用 {level} 級武器時，',
	},
	{
		'pattern': r'(?P<indent>\s*)양손검, 양손창, 양손지팡이, 양손도끼, 카타르, 활, 풍마수리검, 총기 계열의 (?P<level>.+?)레벨 무기인 경우,\s*',
		'replacement': '{indent}使用雙手劍、雙手槍、雙手杖、雙手斧、拳刃、弓、風魔飛鏢與槍械系列的 {level} 級武器時，',
	},
    #endregion
    
    #region 體型傷害
	{
		'pattern': r'(?P<indent>\s*)모든 크기 적에게 주는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對所有體型敵人的物理與魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)모든 크기의 적에게 주는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對所有體型敵人的物理與魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)모든 크기의 적에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對所有體型敵人的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)모든 크기의 적에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對所有體型敵人的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)소형 기의 적에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對小型體型敵人的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)소형 기의 적에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對小型體型敵人的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)중형 기의 적에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對中型體型敵人的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)중형 기의 적에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對中型體型敵人的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)대형 기의 적에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對大型體型敵人的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)대형 기의 적에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對大型體型敵人的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)소형 몬스터에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對小型體型魔物的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)소형 몬스터에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對小型體型魔物的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)중형 몬스터에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對中型體型魔物的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)중형 몬스터에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對中型體型魔物的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)대형 몬스터에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對大型體型魔物的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)대형 몬스터에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對大型體型魔物的魔法傷害 + {value}',
	},
    #endregion
    
    #region 對屬性敵人傷害
	{
		'pattern': r'(?P<indent>\s*)모든 속성 적에게 주는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對所有屬性敵人的物理與魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)모든 속성의 적에게 주는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對所有屬性敵人的物理與魔法傷害 + {value}',
	},
    {
		'pattern': r'(?P<indent>\s*)모든 속성 몬스터에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對所有屬性魔物的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)모든 속성 몬스터에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對所有屬性魔物的魔法傷害 + {value}',
	},
    {
		'pattern': r'(?P<indent>\s*)모든 속성 적에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對所有屬性敵人的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)모든 속성 적에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對所有屬性敵人的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)모든 속성의 적에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對所有屬性敵人的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)모든 속성의 적에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對所有屬性敵人的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)무속성 적에게 주는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對無屬性敵人的物理與魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)무속성 적에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對無屬性敵人的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)무속성 적에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對無屬性敵人的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)화속성 적에게 주는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對火屬性敵人的物理與魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)화속성 적에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對火屬性敵人的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)화속성 적에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對火屬性敵人的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)수속성 적에게 주는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對水屬性敵人的物理與魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)수속성 적에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對水屬性敵人的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)수속성 적에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對水屬性敵人的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)풍속성 적에게 주는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對風屬性敵人的物理與魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)풍속성 적에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對風屬性敵人的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)풍속성 적에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對風屬性敵人的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)지속성 적에게 주는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對地屬性敵人的物理與魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)지속성 적에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對地屬性敵人的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)지속성 적에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對地屬性敵人的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)독속성 적에게 주는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對毒屬性敵人的物理與魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)독속성 적에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對毒屬性敵人的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)독속성 적에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對毒屬性敵人的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)성속성 적에게 주는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對聖屬性敵人的物理與魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)성속성 적에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對聖屬性敵人的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)성속성 적에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對聖屬性敵人的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)암속성 적에게 주는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對暗屬性敵人的物理與魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)암속성 적에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對暗屬性敵人的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)암속성 적에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對暗屬性敵人的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)염속성 적에게 주는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對念屬性敵人的物理與魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)염속성 적에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對念屬性敵人的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)염속성 적에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對念屬性敵人的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)불사속성 적에게 주는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對不死屬性敵人的物理與魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)불사속성 적에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對不死屬性敵人的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)불사속성 적에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對不死屬性敵人的魔法傷害 + {value}',
	},
    #endregion
    
    #region 種族傷害
	{
		'pattern': r'(?P<indent>\s*)모든 종족 몬스터에게 주는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?증가\(플레이어 제외\)',
		'replacement': '{indent}對所有種族魔物的物理與魔法傷害 + {value} (不包含玩家)',
	},
	{
		'pattern': r'(?P<indent>\s*)모든 종족 몬스터에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가\(플레이어 제외\)',
		'replacement': '{indent}對所有種族魔物的物理傷害 + {value} (不包含玩家)',
	},
	{
		'pattern': r'(?P<indent>\s*)모든 종족 몬스터에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가\(플레이어 제외\)',
		'replacement': '{indent}對所有種族魔物的魔法傷害 + {value} (不包含玩家)',
	},
    #endregion
    
    #region 階級傷害
	{
		'pattern': r'(?P<indent>\s*)일반형 적 및 보스형 몬스터에게 주는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對一般階級與 Boss 階級魔物的物理與魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)일반형 적, 보스형 몬스터에게 주는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對一般階級與 Boss 階級魔物的物理與魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)일반형 적 및 보스형 몬스터에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對一般階級與 Boss 階級魔物的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)일반형 적 및 보스형 몬스터에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對一般階級與 Boss 階級魔物的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)일반형 몬스터에게 주는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對一般階級魔物的物理與魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)일반형 몬스터에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對一般階級魔物的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)일반형 몬스터에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對一般階級魔物的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)보스형 몬스터에게 주는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對 Boss 階級魔物的物理與魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)보스형 몬스터에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對 Boss 階級魔物的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)보스형 몬스터에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對 Boss 階級魔物的魔法傷害 + {value}',
	},
    #endregion
    
    #region 近 / 遠 / 暴擊 / 命中
	{
		'pattern': r'(?P<indent>\s*)근접 및 원거리 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}近距離與遠距離物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)근접/원거리 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}近距離與遠距離物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)근접 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}近距離物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)원거리 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}遠距離物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)크리티컬 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}暴擊傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)크리티컬 데미지 (?P<value>.+?) (?:추가\s*)?감소',
		'replacement': '{indent}暴擊傷害 - {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)명중 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}命中物理傷害 + {value}',
	},
    #endregion
    
    #region 屬性法傷
	{
		'pattern': r'(?P<indent>\s*)모든 속성 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}所有屬性的魔法傷害 + {value}',
	},

	{
		'pattern': r'(?P<indent>\s*)무속성 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}無屬性的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)화속성 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}火屬性的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)수속성 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}水屬性的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)풍속성 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}風屬性的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)지속성 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}地屬性的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)독속성 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}毒屬性的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)성속성 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}聖屬性的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)암속성 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}暗屬性的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)염속성 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}念屬性的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)불사속성 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}不死屬性的魔法傷害 + {value}',
	},

	{
		'pattern': r'(?P<indent>\s*)수, 풍, 지속성 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}水、風與地屬性的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)화, 무, 성속성 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}火、無與聖屬性的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)화, 독, 암, 무속성 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가.',
		'replacement': '{indent}火、毒、暗與無屬性的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)풍, 암, 염, 불사속성 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가.',
		'replacement': '{indent}風、暗、念與不死屬性的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)수, 풍, 지, 화, 독, 암, 무속성 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}水、風、地、火、毒、暗與無屬性的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)화, 무, 성, 풍, 암, 염, 불사속성 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}火、無、聖、風、暗、念與不死屬性的魔法傷害 + {value}',
	},
    #endregion
    
    #region 功能
	{
		'pattern': r'(?P<indent>\s*)고정 캐스팅 (?P<value>.+?)초 (?:추가\s*)?감소',
		'replacement': '{indent}固定詠唱時間 - {value} 秒',
	},
	{
		'pattern': r'(?P<indent>\s*)파괴 불가',
		'replacement': '{indent}不會被破壞',
	},
	{
		'pattern': r'(?P<indent>\s*)스킬 사용 시 소모 SP (?P<value>.+?) (?:추가\s*)?감소',
		'replacement': '{indent}施展技能的 SP 消耗 - {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)공격속도(?:추가\s*)?증가\(공격 후 딜레이 (?P<value>.+?) (?:추가\s*)?감소\)',
		'replacement': '{indent}攻擊速度增加 (攻擊後延遲 - {value})',
	},
	{
		'pattern': r'(?P<indent>\s*)공격 속도(?:추가\s*)?증가\(공격 후 딜레이 (?P<value>.+?) (?:추가\s*)?감소\)',
		'replacement': '{indent}攻擊速度增加 (攻擊後延遲 - {value})',
	},
	{
		'pattern': r'(?P<indent>\s*)글로벌 쿨타임 (?P<value>.+?) (?:추가\s*)?감소',
		'replacement': '{indent}共通技能後延遲 - {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)글로벌 쿨타임 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}共通技能後延遲 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)변동 캐스팅 (?P<value>.+?) (?:추가\s*)?감소',
		'replacement': '{indent}變動詠唱時間 - {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)변동 캐스팅 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}變動詠唱時間 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)유도 공격 확률 \+ (?P<value>.+?)',
		'replacement': '{indent}誘導攻擊機率 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)유도 공격 확률 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}誘導攻擊機率 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)근접 물리 데미지 (?P<value_1>.+?), 유도 공격 확률 (?P<value_2>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}近距離物理傷害 + {value_1}、誘導攻擊機率 + {value_2}',
	},
	{
		'pattern': r'(?P<indent>\s*)원거리 물리 데미지 (?P<value_1>.+?), 유도 공격 확률 (?P<value_2>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}遠距離物理傷害 + {value_1}、誘導攻擊機率 + {value_2}',
	},
    #endregion
    
	#region 自動詠唱
	{
		'pattern': r'(?P<indent>\s*)(?P<skill_1>.+?) 사용 시 (?P<skill_2>.+?)가 배운 최대 레벨로 추가 발동\((?P<skill_3>.+?)를 1레벨 이상 배운 경우 발동됩니다\)',
		'replacement': '{indent}施展{skill_1}時自動詠唱已習得最高等級的{skill_2} ({skill_3}習得 1 等級以上時才會發動)',
	},
	{
		'pattern': r'(?P<indent>\s*)일반 근접 물리 공격 시, (?P<value_1>.+?) 확률로 (?P<skill_1>.+?)우 (?P<level_1>.+?)레벨 발동',
		'replacement': '{indent}一般近距離物理攻擊時，有 {value_1} 的機率自動詠唱等級 {level_1} 的 {skill_1}',
	},
	{
		'pattern': r'(?P<indent>\s*)일반 근접 물리 공격 시, (?P<value_1>.+?) 확률로 (?P<skill_1>.+?)우 (?P<level_1>.+?)레벨 발동, (?P<value_2>.+?) 확률로 (?P<skill_2>.+?) (?P<level_2>.+?)레벨 발동',
		'replacement': '{indent}一般近距離物理攻擊時，有 {value_1} 的機率自動詠唱等級 {level_1} 的 {skill_1}、{value_2} 的機率自動詠唱等級 {level_2} 的 {skill_2}',
	},
    #endregion
    
	#region 體型抗性
	{
		'pattern': r'(?P<indent>\s*)모든 크기 적에게 받는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?감소',
		'replacement': '{indent}來自所有體型敵人的物理與魔法傷害 - {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)모든 크기 적에게 받는 물리 데미지 (?P<value>.+?) (?:추가\s*)?감소',
		'replacement': '{indent}來自所有體型敵人的物理傷害 - {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)모든 크기 적에게 받는 마법 데미지 (?P<value>.+?) (?:추가\s*)?감소',
		'replacement': '{indent}來自所有體型敵人的魔法傷害 - {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)중형, 대형 적에게 받는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?감소',
		'replacement': '{indent}來自中型與大型體型敵人的物理與魔法傷害 - {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)중형, 대형 적에게 받는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?감소',
		'replacement': '{indent}來自中型與大型體型敵人的物理傷害 - {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)중형, 대형 적에게 받는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?감소',
		'replacement': '{indent}來自中型與大型體型敵人的魔法傷害 - {value}',
	},
    #endregion
    
	#region 對屬性抗性
	{
		'pattern': r'(?P<indent>\s*)무속성 공격에 대한 내성 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對無屬性的抗性 + {value}',
	},
    #endregion
    
	#region 對屬性敵人抗性
	{
		'pattern': r'(?P<indent>\s*)모든 속성 적에게 받는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?감소',
		'replacement': '{indent}來自所有屬性敵人的物理與魔法傷害 - {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)모든 속성 적에게 받는 물리 데미지 (?P<value>.+?) (?:추가\s*)?감소',
		'replacement': '{indent}來自所有屬性敵人的物理傷害 - {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)모든 속성 적에게 받는 마법 데미지 (?P<value>.+?) (?:추가\s*)?감소',
		'replacement': '{indent}來自所有屬性敵人的魔法傷害 - {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)화속성 적에게 받는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?감소',
		'replacement': '{indent}來自火屬性敵人的物理與魔法傷害 - {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)화속성 적에게 받는 물리 데미지 (?P<value>.+?) (?:추가\s*)?감소',
		'replacement': '{indent}來自火屬性敵人的物理傷害 - {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)화속성 적에게 받는 마법 데미지 (?P<value>.+?) (?:추가\s*)?감소',
		'replacement': '{indent}來自火屬性敵人的魔法傷害 - {value}',
	},
    #endregion
    
	#region 種族抗性
	{
		'pattern': r'(?P<indent>\s*)무형 몬스터에게 받는 물리/마법 데미지 (?P<value>.+?) (?:추가\s*)?감소',
		'replacement': '{indent}來自無形種族魔物的物理與魔法傷害 - {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)무형 몬스터에게 받는 물리 데미지 (?P<value>.+?) (?:추가\s*)?감소',
		'replacement': '{indent}來自無形種族魔物的物理傷害 - {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)무형 몬스터에게 받는 마법 데미지 (?P<value>.+?) (?:추가\s*)?감소',
		'replacement': '{indent}來自無形種族魔物的魔法傷害 - {value}',
	},
    #endregion
    
	#region 階級抗性
    #endregion
    
    #region 雜項
	{
		'pattern': r'(?P<indent>\s*)모든 크기 및 속성 적에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對所有體型與所有屬性敵人的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)모든 크기 및 속성 적에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對所有體型與所有屬性敵人的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)제련도 합의 (?P<value>.+?)배만큼 (?P<skill>.+?) 데미지 % (?:추가\s*)?증가',
		'replacement': '{indent}{skill}的傷害 + (精煉值合計 * {value})%',
	},
	{
		'pattern': r'(?P<indent>\s*)화속성 적, 무형 몬스터에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對火屬性敵人與無形種族魔物的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)화속성 적, 무형 몬스터에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對火屬性敵人與無形種族魔物的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)화속성 적, 무형 몬스터, 중형, 대형 적에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對火屬性敵人、無形種族魔物、中型與大型體型敵人的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)화속성 적, 무형 몬스터, 중형, 대형 적에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對火屬性敵人、無形種族魔物、中型與大型體型敵人的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)중형, 대형 적에게 주는 물리 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對中型與大型體型敵人的物理傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)중형, 대형 적에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對中型與大型體型敵人的魔法傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)중형, 대형 적에게 주는 마법 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}對中型與大型體型敵人的魔法傷害 + {value}',
	},
    #endregion
]

#region Second Layer Regex
SECOND_LAYER_REGEX_REPLACE_RULES = [
	{
		'pattern': r'(?P<indent>\s*)(?P<skill>.+?) 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}{skill}的傷害 + {value}',
	},
	{
		'pattern': r'(?P<indent>\s*)(?P<skill>.+?) 데미지 (?P<value>.+?) (?:추가\s*)?증가',
		'replacement': '{indent}{skill}的傷害 + {value}',
	},
]
#endregion
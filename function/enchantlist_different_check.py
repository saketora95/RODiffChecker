...
def read_file(input_file_path, encoding='utf-8'):
    enchant_file_lines = open(input_file_path, 'r', encoding=encoding).readlines()
    ...
    for line in enchant_file_lines:
        create_index = line.find('CreateEnchantInfo')
        if create_index >= 0:
            if temp_dict['id'] != '':
        ...
        target_index = line.find('AddTargetItem')
        if target_index >= 0:
            temp_dict['target'].append(line.split('"')[1])
...
def compare_dict(old_data_dict, new_data_dict, result_path, encoding='utf-8'):
    ...
    for old_key in old_data_dict:
        if old_key not in new_data_dict:
            relation_equips = ', '.join(old_data_dict[old_key]['target'])
    ...
    for new_key in new_data_dict:
        if new_key in old_data_dict and old_data_dict[new_key] != new_data_dict[new_key]:
            relation_equips = ', '.join(set(old_data_dict[new_key]['target'] + new_data_dict[new_key]['target']))
